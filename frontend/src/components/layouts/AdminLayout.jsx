...
export default function AdminLayout() {
  ...
  const handleSwitchToStudent = async (e) => {
    if (e) {
      e.preventDefault();
      e.stopPropagation();
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to switch role to STUDENT');
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-950 text-slate-100 font-sans selection:bg-indigo-500 selection:text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            <Link to="/admin/dashboard" className="flex items-center gap-2.5 group">
              <div className="p-2 bg-gradient-to-tr from-rose-600 to-indigo-600 rounded-lg shadow-md shadow-rose-500/20 group-hover:scale-105 transition-transform">
                <BrainCircuit className="w-5 h-5 text-white" />
              </div>
              <span className="text-lg font-bold tracking-tight">
                Assessify<span className="text-indigo-400"> AI</span>
                <span className="ml-2.5 text-[10px] uppercase font-bold tracking-wider px-2.5 py-0.5 rounded-full bg-rose-500/10 text-rose-400 border border-rose-500/20 shadow-sm">
                  ADMIN
                </span>
              </span>
            </Link>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              title="Switch role to STUDENT Mode"
            >
              {isSwitching ? (
              ...
              <span>{isSwitching ? 'Switching...' : 'Switch to STUDENT Mode'}</span>
            </button>
            ...
            <div className="pl-2 border-l border-slate-800 flex items-center gap-2">
              <UserButton 
                ...
                appearance={{
                  elements: {
                    avatarBox: 'w-8 h-8 border-2 border-rose-500/40 hover:border-rose-500 transition-colors'
                  }
                }}
              />
            </div>
      ...
      <footer className="border-t border-slate-800/60 py-4 text-center text-xs text-slate-500">
        Assessify AI Admin Portal &bull; Role Authorization Active
      </footer>
    </div>
  );
}
